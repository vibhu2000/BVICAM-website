...
export default function Articles() {
  const [articles, setBulliten] = useState([]);
  const [error, setError] = useState();
  const [loading, setLoding] = useState(true);

  const preload = () => {
    getImages("Articles").then((data) => {
      setLoding(false);
      if (data.error) {
            ...
            {articles.map((articles, index) => {
              return (
                <div className="col-12 col-md-5 col-lg-4 my-3 text-center">
                  <img src={articles.photo} alt="" className="gallery-img w-100 p-0 " style={{backgroundImage:"none",objectFit:'fill'}}/>
